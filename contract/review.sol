...
contract Rating is Ownable {
    ...
    struct Category {
       string title;
       mapping(uint => string) criterias;
    }
    mapping(uint => uint[]) criteriasId;
    uint[] categoriesId;
...
function categoryPresent(uint _categoryId) internal view returns(bool) {
        for(uint i=0;i<categoriesId.length;i++)  {
           if(categoriesId[i] == _categoryId) {
                return true;
            } else {
                return false;
            }
    ...
    function addProduct(string memory _title , uint _category_id) public  {
    ...
    function addCategory(string memory _title ,uint _categoryId) public  {
    ...
    function addCriteria(uint _categoryId , string memory _criteria , uint _criteriaId) public onlyOwner {
         require(categoryPresent(_categoryId) == true, "No such category with given id is present.");
    ...
    function addReview(uint _productId, uint[] memory ratings) public  returns(string memory result) {
        ...
        string memory categoryId = uint2str(product.category_id);
        uint avgRating = 0;
        ...
        for(uint i=0;i<criteriasId[product.category_id].length;i++) {
            uint id = criteriasId[product.category_id][i];
        ...
        product.sumRating = ((product.sumRating*(product.reviewsCount-1))+avgRating)/product.reviewsCount;
   ...
   function getRatings(uint _productId , uint index) private view returns(string memory result) {
       Product storage product = products[_productId];
       string memory _categoryId = uint2str(product.category_id);
