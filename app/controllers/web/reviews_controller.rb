class Web::ReviewsController < ApplicationController
	...
	def get_reviews
		...
		if @review.save
			criteria = JSON.parse(@product.reviews.first.criteria.gsub("=>",":")).values 
        	review = contract_instance.transact_and_wait.add_review(@product.product_blockchain_id,criteria) rescue nil
			Blockchain.create!(criteria: JSON.parse(@product.reviews.first.criteria.gsub("=>",":")), mined: review.as_json["mined"], connection: review.as_json["connection"],blockchain_hash: review.as_json["id"],product_blockchain_id: @product.product_blockchain_id,rating: @review.rating, product_id: params[:id], user_id:  current_user.id)
